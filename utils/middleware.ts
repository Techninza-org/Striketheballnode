...
const UserMiddleware = async (req: ExtendedRequest, res: Response, next: NextFunction) => {
    const token = req.headers?.authorization
    ...
    try {
        const user = await prisma.customer.findFirst({where: {phone}})
        if (!user) {
...
const middleware = { ErrorHandler, AuthMiddleware, AdminMiddleware, UserMiddleware }
